# ═══════════════════════════════════════════════════════════════════════
#  Phone Cleanup, publish a new release
#
#  Usage:
#     .\publish.ps1                       (prompts for version + notes)
#     .\publish.ps1 -Version 0.2.0 -Notes "Duplicate scan is twice as fast"
#
#  What it does:
#     1. Bumps the version in pubspec.yaml (and the build number)
#     2. Commits the change
#     3. Creates a v<version> tag carrying your release notes
#     4. Pushes, which triggers GitHub Actions to build the signed APK
#        and publish it as a Release the phone can pull.
#
#  On the phone: open the app. If a release is newer than what is installed,
#  a line appears at the top of the Storage tab. One tap installs it.
# ═══════════════════════════════════════════════════════════════════════
param(
    [string]$Version,
    [string]$Notes
...
# --- get target version ---
if (-not $Version) {
    $Version = Read-Host "New version number (e.g. 0.2.0) [current: $currentName]"
}
if ($Version -notmatch '^\d+\.\d+\.\d+$') {
    Write-Error "Version must look like 0.2.0"; exit 1
}
...
Write-Host "Watch progress:  https://github.com/scenicprints/phone-cleanup/actions" -ForegroundColor Cyan
Write-Host "When it finishes, open the app. The update offers itself." -ForegroundColor Cyan
